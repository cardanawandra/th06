...
#include "utils.hpp"
#include <map>
std::map<int, BITS_16> g_ctrl_bits_self;
std::map<int, BITS_16> g_ctrl_bits_rcved;
...
void Controller::SendKeys(int frame)
{
    ...
    for (int i = 0; i < KeyPackFrameNum; i++)
    {
        std::map<int, BITS_16>::iterator find_res = g_ctrl_bits_self.find(frame - i);
...
u16 GetKeys(int frame, bool is_in_UI, int &out_ctrl)
{
    ...
    u16 self_key = 0;
    std::map<int, BITS_16>::iterator res = g_ctrl_bits_self.find(frame - g_delay);
...
u16 Controller::GetInput_Net(int frame, bool is_in_UI, int &cur_ctrl)
{
    ...
    // remove frames
    int frame_rem = 80;
    std::map<int, BITS_16>::iterator last_res = g_ctrl_bits_self.find(frame - frame_rem);
