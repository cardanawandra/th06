...
GfxInterface *WebGL::Create()
{
    ...
    // Optional: if you want consistent 2D scaling behavior
    SetConfigFlags(FLAG_VSYNC_HINT);

    // ----------------------------------------------------
    // Create window (replaces SDL_CreateWindow + GL context)
    // ----------------------------------------------------
    InitWindow(width, height, TH_WINDOW_TITLE);

    if (!IsWindowReady())
    {
        LOG_COMPAT("Failed to create Raylib window\n");
        delete gfx;
        return NULL;
    }
    ...
    // ----------------------------------------------------
    // Initialize renderer backend (your existing system)
    // ----------------------------------------------------
    if (!gfx->Init())
    {
        CloseWindow();
