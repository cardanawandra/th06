...
GfxInterface* Software::Init()
{
    ...
    GAME_WINDOW_WIDTH_REAL  = 640;
    GAME_WINDOW_HEIGHT_REAL = 480;
...
int RGBToConio(u32 color)
{
    ...
    if (r > 200) return LIGHTRED;
    if (g > 200) return LIGHTGREEN;
    if (b > 200) return LIGHTBLUE;

    if (r > 100 && g > 100 && b > 100) return LIGHTGRAY;
...
{
    int sx = GAME_WINDOW_WIDTH_REAL / 80;
    int sy = GAME_WINDOW_HEIGHT_REAL / 25;
    ...
    for (int y = 0; y < 25; y++)
    {
        ...
        {
            u32 pixel =
                framebuffer[(y * sy) * GAME_WINDOW_WIDTH_REAL + (x * sx)];

            int c = RGBToConio(pixel);
