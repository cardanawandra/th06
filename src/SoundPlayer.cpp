...
ZunResult SoundPlayer::InitializeDSound()
{
    ...
    if (SDL_InitSubSystem(SDL_INIT_AUDIO) != 0){
        // LOG_COMPAT("SDL_INIT_AUDIO failed: %s\n",SDL_GetError());
        // return ZUN_ERROR;
    }
...
ZunResult SoundPlayer::LoadWav(const char *path)
{
    ...
    if (fileStream == NULL)
    {
        LOG_COMPAT("error : wav file load error %s\n", path);
        return ZUN_ERROR;
    }
...
ZunResult SoundPlayer::LoadSound(i32 idx, const char *path, f32 volumeMultiplier)
{
    ...
    LOG_COMPAT("load sound 3\n");
    wavRawData = (u8 *)FileSystem::OpenPath(path, 0);

    if (wavRawData == NULL)
    {
        LOG_COMPAT("load sound fail 16\n"); return ZUN_ERROR;
    }

    LOG_COMPAT("load sound 4\n");
...
{
    DISABLE_SOUNDPLAYER;
    LOG_COMPAT("play Sound by index\n");
    for (int i = 0; i < 3; i++)
    {
