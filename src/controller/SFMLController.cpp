...
u16 Controller::GetControllerInput(u16 buttons)
{
    ...
    if (g_Supervisor.joystick != NULL)
    {
        ...
        buttons |= JOYSTICK_BUTTON_PRESSED(TH_BUTTON_RIGHT, stickX, JOYSTICK_MIDPOINT(0, INT16_MAX_COMPAT));
        buttons |= JOYSTICK_BUTTON_PRESSED(TH_BUTTON_LEFT, -stickX, JOYSTICK_MIDPOINT(0, INT16_MAX_COMPAT));

        buttons |= JOYSTICK_BUTTON_PRESSED(TH_BUTTON_DOWN, stickY, JOYSTICK_MIDPOINT(0, INT16_MAX_COMPAT));
        buttons |= JOYSTICK_BUTTON_PRESSED(TH_BUTTON_UP, -stickY, JOYSTICK_MIDPOINT(0, INT16_MAX_COMPAT));
        //
...
const u8 *Controller::GetControllerState()
{
    if (g_Supervisor.joystick != NULL)
    {
        ...
        Joystick *joystick = JOYSTICK_COMPATGetJoystick(g_Supervisor.joystick);
