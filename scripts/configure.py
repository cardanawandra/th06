...
def configure(build_type):
    # Modern MSVC toolchain paths (computed early, used throughout)
    _msvc_root  = r"C:\Program Files\Microsoft Visual Studio\18\Community\VC\Tools\MSVC\14.44.35207"
    _winsdk     = r"C:\Program Files (x86)\Windows Kits\10"
    _winsdk_ver = "10.0.26100.0"
    _cl_modern   = _msvc_root + r"\bin\HostX86\x86\cl.exe"
    _link_modern = _msvc_root + r"\bin\HostX86\x86\link.exe"
    _lib_modern  = (
        _msvc_root + "\\lib\\x86"
        + ";" + _winsdk + "\\Lib\\" + _winsdk_ver + "\\ucrt\\x86"
        + ";" + _winsdk + "\\Lib\\" + _winsdk_ver + "\\um\\x86"
    )
    ...
    with (SCRIPTS_DIR.parent / "build.ninja").open("w") as f:
        ...
        cl_common_flags = "/MT /EHsc /G5 /Gs /GS /DNDEBUG /Zi /I $builddir/autogenerated /I src /I 3rdparty/munit /I 3rdparty/Detours/src /I 3rdparty/imgui"
        ...
        # Use the modern MSVC linker so it can resolve modern runtime symbols
        # from ImGui objects compiled with modern cl.exe.
        writer.variable("link", '"' + _link_modern + '"')
        _th06_link_libpath = (
            ' /LIBPATH:"' + _msvc_root + '\\lib\\x86"'
            + ' /LIBPATH:"' + _winsdk + '\\Lib\\' + _winsdk_ver + '\\ucrt\\x86"'
            + ' /LIBPATH:"' + _winsdk + '\\Lib\\' + _winsdk_ver + '\\um\\x86"'
        )
        writer.variable(
            "th06_link_flags",
            "/subsystem:windows /machine:X86 /incremental:no /opt:ref /map"
            + _th06_link_libpath,
        )
        ...
        # ImGui: must be compiled with a C++14-capable compiler (game uses VC7/C++98).
        # _msvc_root, _winsdk, _winsdk_ver, _cl_modern, _link_modern are defined
        # at the top of configure() so they can be used for the link rule too.
        _dxsdk = r"$builddir/../scripts/prefix/mssdk"  # th06 DX8 SDK for d3d8.h
        writer.variable("cl_modern", '"' + _cl_modern + '"')
        writer.variable(
            "cl_flags_imgui",
            '/nologo /MT /EHsc /GS /DNDEBUG /Zi /Od /std:c++14 /utf-8'
            ' /I 3rdparty/imgui'
            ' /I src'
            ' /I $builddir/autogenerated'
            ' /I "' + _msvc_root + '\\include"'
            ' /I "' + _winsdk + '\\Include\\' + _winsdk_ver + '\\ucrt"'
            ' /I "' + _winsdk + '\\Include\\' + _winsdk_ver + '\\um"'
            ' /I "' + _winsdk + '\\Include\\' + _winsdk_ver + '\\shared"'
            ' /I scripts/prefix/mssdk/include'
        )
        writer.rule(
            "cc_imgui",
            "$cl_modern /showIncludes $cl_flags_imgui /c $in /Fd$out.pdb /Fo$out",
            deps="msvc",
        )
        imgui_sources = [
            "imgui",
        ...
        for rule in imgui_sources:
            writer.build(
                "$builddir/" + rule + ".obj",
                "cc_imgui",
                "3rdparty/imgui/" + rule + ".cpp",
            )
        # ThpracGui and ThImGuiBridge use imgui.h (C++14), compile with modern MSVC.
        for src in ["ThpracGui", "ThImGuiBridge"]:
            writer.build(
                "$builddir/" + src + ".obj",
                "cc_imgui",
                "src/" + src + ".cpp",
        ...
        # legacycrt provides _vsprintf/_sprintf/__snprintf stubs missing from modern libcmt.
        writer.build(
            "$builddir/legacycrt.obj",
            "cc_imgui",
            "src/legacycrt.cpp",
        )
        ...
        objfiles = (
            ...
            + ["$builddir/" + src + ".obj" for src in imgui_sources]
            + ["$builddir/ThpracGui.obj", "$builddir/ThImGuiBridge.obj"]
            + ["$builddir/legacycrt.obj"]
        ...
        # Modern MSVC runtime libs satisfy symbols from ImGui (C++14) objects.
        # legacycrt.obj provides _vsprintf/_sprintf/__snprintf stubs for VC7-compiled code.
        th06_link_libs = "dxguid.lib d3dx8.lib d3d8.lib dsound.lib winmm.lib kernel32.lib user32.lib dinput8.lib gdi32.lib winspool.lib comdlg32.lib advapi32.lib shell32.lib ole32.lib oleaut32.lib uuid.lib odbc32.lib odbccp32.lib libcmt.lib libvcruntime.lib libucrt.lib"
        writer.build(
            "$builddir/th06e.exe",
            "link",
            inputs=objfiles,
